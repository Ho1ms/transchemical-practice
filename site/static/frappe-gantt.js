// author: https://github.com/frappe/gantt
...
//        document.querySelector(`#${task_id}-highlight`).style.display = 'none';
  ...
  class Gantt {
    ...
    setup_options(options) {
      ...
      if ("tasks_name" in options) {
        this.tasks_name = options['tasks_name']
      };
    ...
    setup_layers() {
      this.layers = {};
      const layers = ["grid", "arrow", "progress", "bar", "details", 'names'];
      // make group layers
    ...
    set_tasks_name () {
        const text_layer = createSVG("g", { append_to: this.layers.names });
      ...
      [...Array((this.counter_rows > -1 ? this.counter_rows : this.tasks.length))].forEach((e,i) => {
        // TODO сюда подгружать список сотрудников

        createSVG("text", {
        x: 10,
        y: (row_y + 24) ,
        innerHTML: this.tasks_name[i],
        class: "bar-label",
    ...
    make_grid() {
      let useCondensedRows = false;
            ...
            if (useCondensedRows) {
                //counter_rows = 0
                counter_rows = this.tasks_name.length;
            }
            this.counter_rows = counter_rows
    ...
    make_grid_background(lenRows) {
      ...
      const grid_height =
        ...
        (this.options.bar_height + this.options.padding) * (lenRows > -1 ? lenRows : this.tasks.length);
    ...
    make_grid_ticks() {
      ...
      let tick_height =
        (this.options.bar_height + this.options.padding) * this.counter_rows;
      ...
      if (this.options.lines !== 'vertical') {
        for (let _ of Array(this.counter_rows).keys()) {
